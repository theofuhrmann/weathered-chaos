...
class RAVESonifier:
    ...
    def generate_latents(self) -> torch.Tensor:
        """
        Generate latent vectors from the pendulum system's dynamics. The latent
        vectors are generated by encoding the pendulum positions and angular
        velocities into a tensor, which are then stacked by groups of the latent
        dimension. If there are not enough latent vectors, the remaining ones
    ...
    def generate_audio(self, latents: torch.Tensor) -> np.ndarray:
        """
        Generate audio from the latent vectors. The latent vectors are decoded
        into audio using the pre-trained torch model.
        """
        with torch.no_grad():
